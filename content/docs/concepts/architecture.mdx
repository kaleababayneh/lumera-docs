...
File size is rounded **up** to the next whole kilobyte (`(bytes + 1023) / 1024`), matching the Supernode-side verification. Both values are on-chain `x/action` parameters (adjustable by governance); the current mainnet values are:
