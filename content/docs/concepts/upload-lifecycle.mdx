...
A Cascade upload consists of three phases: **prepare**, **register**, and **send**. The SDK's `uploadFile` method handles all three phases in sequence, but you can also run them individually for fine-grained control.
...
After the file is successfully encoded and distributed, the receiving Supernode broadcasts a `MsgFinalizeAction` transaction to the chain. This transitions the action state from `PENDING` to `DONE` and triggers fee distribution to the participating Supernodes. The creator can optionally approve the action via `MsgApproveAction`, which moves it to the `APPROVED` state.
