...
The [Go SDK](https://github.com/LumeraProtocol/sdk-go) is the official SDK for building server-side applications, CLI tools, and backend services that interact with Lumera Protocol. It needs gRPC access to a chain endpoint and direct network access to Supernodes.
...
Unlike the [JavaScript](/docs/getting-started/javascript-sdk) and [Rust](/docs/getting-started/rust-sdk) SDKs which route through SN-API (a REST gateway), the Go SDK communicates **directly with Supernodes** over gRPC:
