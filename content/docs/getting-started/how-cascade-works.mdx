...
Once preparation is complete, the SDK submits a **`MsgRequestAction`** transaction to the Lumera chain. This message includes `data_hash`, `file_name`, `visibility flag` indicating whether the file is public or private, and the one-time storage fee in `ulume`.
