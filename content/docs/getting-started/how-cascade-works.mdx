...
2. Generates a **RaptorQ layout** — metadata describing how the file will be encoded, not the encoding itself.
3. **Signs** the layout, derives deterministic **layout IDs** (LEP-1), and builds a signed **index file**.

Once preparation is complete, the SDK submits a **`MsgRequestAction`** transaction to the Lumera chain. This message includes `data_hash`, `file_name`, a `visibility flag` indicating whether the file is publicly or privately accessible, and the one-time storage fee in `ulume`.
...
After encoding and distribution are complete, the Supernode submits a **`MsgFinalizeAction`** transaction back to the chain. This transitions the action state from `PENDING` to `DONE`, permanently recording that the file is stored and retrievable. The escrowed fee is then distributed: **90% to participating Supernodes** and **10% to the foundation**.

---

At this point, your file is **permanently stored**. To retrieve it, the SDK signs the `action_id` with ADR-036 and authenticates with the Supernode network to stream the reconstructed file back to your application.
