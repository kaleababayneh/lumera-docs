...
The [Rust SDK](https://github.com/LumeraProtocol/sdk-rs) is the official SDK for building high-performance server-side applications, CLI tools, and system-level integrations with Lumera Protocol. It provides native gRPC chain access and SN-API interaction without WASM overhead.
...
**Requirements**: Rust stable 1.75+, secp256k1 signing keys, reachable Lumera endpoints.
...
Or load configuration from environment variables, `.env`, `.toml`, or `.json` files:
