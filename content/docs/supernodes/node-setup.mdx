...
First, on the **validator**, confirm `lumera` is actually listening on all interfaces:

```bash
sudo ss -tlnp | grep 9090
```

You should see a line containing `*:9090` or `0.0.0.0:9090`. If it still shows `127.0.0.1:9090`, the edit to `app.toml` did not take effect — re-check the file and restart `lumera` again before continuing.

Then, on the **Supernode server**, test that gRPC is reachable from the other side:

```bash
curl -v telnet://<VALIDATOR_IP>:9090 --max-time 3 2>&1 | head -5
```

You should see `Connected to <VALIDATOR_IP> port 9090`.
