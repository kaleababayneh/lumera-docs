...
<YouTube id="8YC3KRBi-eU" title="Lumera Supernode Setup" />

## Architecture

<Diagram name="supernode-architecture" />

The Supernode and its validator are two **independent hosts that meet on-chain**. The validator produces blocks on Lumera through its normal consensus P2P on port 26656. The Supernode separately talks to the Lumera chain via gRPC to read state and broadcast its own messages.
...
| gRPC endpoint | Public testnet gRPC (see [Community Endpoints](/docs/resources/community)) or self-hosted `<VALIDATOR_IP>:9090` | Public mainnet gRPC (see [Community Endpoints](/docs/resources/community)) or self-hosted `<VALIDATOR_IP>:9090` |
| Min stake | 10,000 LUME combined (self-delegation + SN account) | 25,000 LUME combined (self-delegation + SN account) |
