...
Every Supernode is paired with a validator but **must run on a separate server**. The Supernode maintains its own account, keys, and stake delegation, and talks to the Lumera chain over gRPC — typically through a public testnet endpoint rather than directly to its own validator.

## Architecture

<Diagram name="supernode-architecture" />

The Supernode and its validator are two **independent hosts that meet on-chain**, not two machines wired directly together. The validator produces blocks on Lumera through its normal consensus P2P on port 26656. The Supernode separately talks to the Lumera chain via gRPC — through a public testnet endpoint — to read state and broadcast its own messages. Neither host makes a persistent network connection to the other.
...
Together those two facts form the entire Supernode ↔ validator relationship. Supernodes also form a peer-to-peer mesh with *other Supernodes* using a Kademlia DHT (port 4445) for data replication and storage challenge proofs across the network — again, a chain-wide link, not a wire to any particular validator.
...
<Callout type="info">
Most operators do **not** need to open port 9090 on their validator. By default, the Supernode uses a public testnet gRPC endpoint and never talks to your validator directly at runtime. Only expose 9090 if you are following the self-host option in [Node Setup, Step 3](/docs/supernodes/node-setup#step-3-choose-a-lumera-grpc-endpoint), and if you do, restrict it to your Supernode's IP only.
</Callout>
