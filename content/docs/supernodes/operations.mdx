...
description: Day-to-day Supernode operations — monitoring, lifecycle management, upgrades, and troubleshooting.
icon: Wrench
---
...
## On-Chain Metrics and Audit (LEP-4)

The chain's **Audit module** (`x/audit`) enforces Supernode compliance using an epoch-based system (default: 400 blocks per epoch).

Each epoch, every Supernode must submit a `MsgSubmitEpochReport` containing:
- **Host metrics**: version, CPU/RAM/disk usage, uptime, port status
- **Peer observations**: connectivity checks on assigned peer Supernodes

If metrics are not reported, fall below minimums (8 CPU cores, 16 GB RAM, 1 TB storage), or peers report closed ports, the Supernode is moved to `POSTPONED` state at the epoch boundary. Recovery requires submitting a compliant self-report and receiving peer confirmation.

This reporting is handled automatically by `sn-manager` — no manual action is needed.
