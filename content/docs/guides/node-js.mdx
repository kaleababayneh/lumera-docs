...
The `@lumera-protocol/sdk-js` SDK works in both **browser** and **Node.js** environments. This guide covers the Node.js-specific setup for server-side applications, CLI tools, and backend services where programmatic signing with `DirectSecp256k1HdWallet` replaces browser wallet extensions.

For server-side applications that do not need WASM, consider the native [Go SDK](/docs/getting-started/go-sdk) or [Rust SDK](/docs/getting-started/rust-sdk) as alternatives.
