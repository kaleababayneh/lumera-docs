...
description: Reusable patterns for any Injective × Cascade product — the action_id pointer, contract-side conventions, indexer responsibilities, frontend patterns, and the failure modes to design around.
icon: Construction
---

[Inscribe](/docs/injective/inscribe) is one shape of Injective × Cascade product. It is not the only one. This page is the recipe book for building a different one: the architecture every integration shares, the conventions that make it clean, and the pitfalls worth a deliberate decision up front.
