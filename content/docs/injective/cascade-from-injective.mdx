...
Cascade's unit of storage is an **action**. A `MsgRequestAction` on Lumera registers an artifact's metadata and returns a numeric `action_id`. Once stored, the artifact is retrievable forever by `GET /download/<action_id>` against any cascade-aware gateway. See [How Cascade Works](/docs/getting-started/how-cascade-works) for the full lifecycle.
...
<Callout type="warn">
**This is not on the hot path, and you should not reach for it first.** An earlier prototype of the reference implementation dispatched Cascade writes this way via [`cw-ica-controller`](https://github.com/srdtrk/cw-ica-controller) and waited for a Hermes relayer. Reserve Pattern C for canonical, contract-only artifacts where there is genuinely no user in the loop and the *contract itself* must be the signer.
</Callout>
