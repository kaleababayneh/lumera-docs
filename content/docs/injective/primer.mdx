...
description: A short, integration-focused tour of Injective — the chain facts, networks, CosmWasm, and the Keplr wallet setup that matter when wiring an Injective dApp to Cascade.
icon: BookOpen
---
...
<Callout type="warn">
**Amino only.** Injective's `eth_secp256k1` keys are not direct-mode-friendly under the current cosmjs build. Use `keplr.getOfflineSignerOnlyAmino(chainId)` rather than `getOfflineSigner`. If you use `SigningCosmWasmClient` with the standard direct-mode signer, you'll get opaque signature-mismatch errors.
</Callout>
...
The recommended integration paths — server-signed via the cascade-api, or user-signed from a wallet — have no IBC step at all. Treat ICA as an advanced option reserved for the narrow case where a *contract itself* must be the signer of a canonical write with no user in the loop. It is covered, with that framing, in [Cascade from Injective](/docs/injective/cascade-from-injective#pattern-c-contract-driven-via-ica-advanced).
